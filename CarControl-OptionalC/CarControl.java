...
class Alley {
	...
	volatile int cStreak; //clockwise alley streaks
	volatile int ccStreak; //counter-clockwise alley streaks
	...
	public synchronized void enter(int no){
		if (no < 5){
			...
			cStreak = 0; //reset other streak counter
			ccCounter++; //enter
			ccStreak++; //count streak
		} else {
			while(ccCounter > 0 || cStreak >= 8){
				try {
					wait(); //wait for access
				} catch (InterruptedException e) {
					e.printStackTrace();
			...
			ccStreak = 0; //reset other streak counter
			cCounter++; //enter
			cStreak++; //count streak
		}
	}
